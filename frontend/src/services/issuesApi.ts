...
export const issuesApi = {
  list(params: IssueListParams) {
    return apiClient.get<CollectionResponse<IssueSummary>>('/issues', params as Record<string, unknown>);
  },
  getById(id: string, includeArchived = false) {
    return apiClient.get<SingleResponse<IssueDetail>>(`/issues/${id}`, { include_archived: includeArchived });
  },
  create(payload: Required<Pick<IssueUpsertInput, 'title' | 'project_id'>> & IssueUpsertInput) {
    return apiClient.post<SingleResponse<IssueDetail>>('/issues', payload);
  },
  update(id: string, payload: IssueUpsertInput) {
    return apiClient.put<SingleResponse<IssueDetail>>(`/issues/${id}`, payload);
  },
  archive(id: string) {
